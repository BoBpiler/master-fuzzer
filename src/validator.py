...
def check_for_duplicated_bug(dir_path, generator_config, id, random_seed):
    ...
    is_duplicated_by_ULL = detect_bug_type_ULL(dir_path, generator_config, id, random_seed)
...
def analyze_results_for_duplicate(generator_config, id, random_seed, results, machine_info, partial_timeout=True):
    # 해당 결과들을 대상으로 비교
    generator_name = generator_config['name']
    try:
...
def fuzz(dir_path, generator_config, id, random_seed, partial_timeout=True):
    ...
    if len(results) > 0: 
        result = analyze_results_for_duplicate(generator_config, id, random_seed, results, machine_info, partial_timeout)
    else:
