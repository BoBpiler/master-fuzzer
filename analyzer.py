...
def analyze_results(generator, id, results, machine_info, partial_timeout=True):
    ...
    try:
        if compare_execution_results(results):  # 실행 결과가 다른 경우
            ...
            send_telegram_message(machine_info, generator, id, "Different Checksum", msg, id_folder_path, os.path.join(id_folder_path, f"{id}_result.txt"), "high")
        else:
            ...
            if crash_exists:                                    # 크래시가 있는 경우
                ...
                send_telegram_message(machine_info, generator, id, f"{crash_type} Crash", msg, id_folder_path, os.path.join(id_folder_path, f"{id}_result.txt"), "medium")
                ...
                send_telegram_message(machine_info, generator, id, "Abnormal Compile", msg, id_folder_path, os.path.join(id_folder_path, f"{id}_result.txt"))
...
def save_to_folder(generator, id, results, folder_name):
    ...
    if generator == 'yarpgen':
        ...
        shutil.move(f"{TEMP_DIRS[generator]}/{id}/{id}.c", os.path.join(id_folder_path, f"{id}.c"))
