...
def generate_c_code(id, generator):
    try:
        ...
        if generator == 'csmith':
            ...
            # C 코드 생성
            subprocess.run(['csmith', '-o', filepath], env=csmith_env, stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=time_out)
            return filepath
        elif generator == 'yarpgen':
            subprocess.run(['yarpgen', '--std=c', '-o', TEMP_DIRS[generator]], stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=time_out)
